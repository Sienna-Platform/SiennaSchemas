...
| Inline schema aliases | PowerOpenAPIModels | `make validate` (`test/validate.jl`) | a shared schema silently duplicated as `<Base>1`, `<Base>2`, … because an inline object at the reference site had no named `$defs` entry. Now prevented at the source — inline objects are named in the schemas and the bundler emits an internal `$ref` for anything a selector already publishes rather than inlining a second copy — this gate is the downstream backstop: alias names are assigned by OpenAPI.jl's native generator and cannot be derived statically from the schemas alone, so a check here would have false negatives. Keys on the unsuffixed base existing, so real digit-suffixed type names (`SteamTurbineGov1`) are not flagged |
