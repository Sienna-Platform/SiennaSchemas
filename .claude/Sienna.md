...
4. The workspace top-level `CLAUDE.md` (parent of both lines) — the two-line workspace layout and git/test policy.
...
- **Shared dev env:** `julia --project=<psy6-workspace-root>` dev-wires all co-developed packages (rebuild with `psy6/wire_psy6.jl`). A dev'd dependency's own `[sources]` pins are ignored by the parent env — only the active project's count; that is why the shared env exists.
